...
> **Source:** Content from [Tofugu](https://www.tofugu.com/japanese-grammar/kono-sono-ano-dono/). For personal study.

Japanese Demonstratives: この(Kono) • その(Sono) • あの(Ano) • どの(Dono)
- 
...
この, その, あの, and どの are a set of こそあど言葉 (ko-so-a-do words). They are placed before nouns to give information about their location in relation to you, as in このトマト (this tomato), そのトマト (that tomato), あのトマト (that tomato over there), and どのトマト (which tomato?)
...
この, その, あの, and どの are added to the beginning of nouns. In the following example, each word is combined with the noun スカート (skirt).
  ...
  - **この**スカートと**その**スカートと**あの**スカートだったら、**どの**スカートがいい？
...
Japanese words don't usually change depending on whether you are talking about one thing or multiple things. For example, トマト (tomato) is still トマト in Japanese even when you are talking about three tomato**es**.
...
There are the plural versions of この, その, and あの, which are combinations of これら, それら, and あれら and the particle の, as in これらの, それらの, and あれらの.1  However, these words carry an explanatory tone and are generally used in more formal speaking or writing, as in:
  ...
  - ブドウ、ブルーベリー、トマト…**これらの**果物にはポリフェノールが多く含まれている。
...
And if you want to recommend a few items of interest on your YouTube channel, you may use その with numbers to introduce each item:
  ...
  - おすすめアイテム、その１は、この「トマトスライサー」です。…その２は、…その３は、…
...
In writing, the author may use あの to add a similar effect. For example, the following is from the official outline of an episode of the anime  戦場(せんじょう)のヴァルキュリア (Valkyria Chronicles):

  -  幼馴染(おさななじ)みのスージーと 義勇軍(ぎゆうぐん)へ 入隊(にゅうたい)したアリシアは、 任命式(にんめいしき)で 第(だい)7 小隊(しょうたい)への 配属(はいぞく)が 決(き)まる。なんと 隊長(たいちょう)は**あの**ウェルキン。
...
<!-- quiz: {"type":"multiple-choice","question":"What does \"**この**スカートと**その**スカートと**あの**スカートだったら、**どの**スカートがいい？\" mean?","options":["Out of **this** skirt, **that** skirt, and **that** skirt over there, **which** one do you like the best?","I like **that** red skirt.","I like **that** [red frilly] skirt [on the right].","Look! **This** tomato is so big."],"answer":0} -->
...
- [Tofugu](https://www.tofugu.com/japanese-grammar/kono-sono-ano-dono/)
