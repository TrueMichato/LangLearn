...
export default function ListeningCard({ word, isFlipped, onFlip }: ListeningCardProps) {
  if (!isTTSSupported()) {
    return (
      <div className="w-full min-h-[240px] rounded-2xl shadow-lg p-6 flex flex-col items-center justify-center bg-white dark:bg-slate-800 border border-yellow-200 dark:border-yellow-700">
  ...
  return (
    ...
    >
      ...
      <p className="text-lg text-slate-600 dark:text-slate-300 mb-3">What did you hear?</p>
      ...
      {isFlipped ? (
        <div className="mt-2 text-center">
          ...
          {word.reading && (
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">{word.reading}</p>
          )}
